...
class DecoderBlock(nn.Module):
    ...
    def forward(self, X: torch.Tensor, Y_enc : torch.Tensor, len_enc : torch.Tensor, K : torch.Tensor = None) -> torch.Tensor:
...
class Decoder(nn.Module):
    def __init__(self, vocab_size,
        ...
        self.num_hiddens = n_hidden
        # Decoder key states
        self.K_states = [None] * n_blocks

    def forward(self, X: torch.Tensor, Y_enc : torch.Tensor, len_enc : torch.Tensor) -> torch.Tensor:
        # X[batch_size, seq_len], Y_enc[batch_size, seq_len, n_hidden], len_enc[batch_size]
        X = self.embedding.forward(X) * math.sqrt(self.num_hiddens)
        X = self.encoding.forward(X)        
        for i, block in enumerate(self.blocks):
            block : DecoderBlock            
            X, self.K_states[i] = block.forward(X, Y_enc, len_enc, self.K_states[i])
